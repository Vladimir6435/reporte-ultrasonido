...
// ============================================================
//  Word de la hoja de hitos del embarazo
// ============================================================
export async function generateHitosDocx(state, egRes) {
  ...
  children.push(
    new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [new TextRun({ text: "Hitos del embarazo — Fechas clave personalizadas", bold: true, size: 28, color: BRAND })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 120 },
      border: { bottom: { style: BorderStyle.SINGLE, size: 6, color: BRAND, space: 6 } },
      children: [new TextRun({ text: "Calendario informativo de su control prenatal", size: 18, color: "555555" })],
    })
  );

  if (d.paciente)
    children.push(new Paragraph({ spacing: { after: 30 }, children: [new TextRun({ text: "Paciente: ", bold: true, size: 20 }), new TextRun({ text: d.paciente, size: 20 })] }));
  if (d.eg)
    children.push(new Paragraph({ spacing: { after: 160 }, children: [new TextRun({ text: "Edad gestacional: ", bold: true, size: 20 }), new TextRun({ text: d.eg + (d.fpp ? `   ·   Fecha probable de parto: ${d.fpp}` : ""), size: 20 })] }));
  ...
  d.items.forEach((it) => {
    children.push(
      new Paragraph({
        ...
        children: [
          new TextRun({ text: `${it.titulo}  —  `, bold: true, size: 22, color: "123A68" }),
          new TextRun({ text: it.fecha + (it.superada ? " (ya superada)" : ""), bold: true, size: 22, color: BRAND }),
        ],
      }),
      new Paragraph({ alignment: AlignmentType.JUSTIFIED, spacing: { after: 40 }, children: [new TextRun({ text: it.leyenda, size: 20 })] })
    );
  });

  children.push(
    new Paragraph({ spacing: { before: 200 }, alignment: AlignmentType.JUSTIFIED, children: [new TextRun({ text: d.cierre, size: 20 })] }),
    new Paragraph({ spacing: { before: 220 }, border: { top: { style: BorderStyle.SINGLE, size: 6, color: BRAND, space: 6 } }, children: [new TextRun({ text: d.consultorio, bold: true, size: 20, color: BRAND })] }),
    new Paragraph({ children: [new TextRun({ text: d.contacto, size: 20 })] })
  );
  ...
  const blob = await Packer.toBlob(doc);
  const nombre = (d.paciente || "paciente").normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/[^A-Za-z0-9]/g, "");
  saveAs(blob, `Hitos_${nombre}.docx`);
}
