...
function alertBlock(label, value) {
  return new Paragraph({
    ...
    shading: { fill: "FDE8E8" },
    border: {
      left: { style: BorderStyle.SINGLE, size: 24, color: "C81E1E", space: 6 },
    },
    children: [
      new TextRun({ text: `${label}: `, bold: true, size: 22, color: "C81E1E" }),
      new TextRun({ text: String(value), bold: true, size: 22, color: "7A1212" }),
    ],
  });
}
...
export async function generateDocx(state) {
  ...
  children.push(
    ...
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: `${r.encabezado.especialidad1}  ·  ${r.encabezado.especialidad2}`,
  ...
  children.push(
    ...
    new Paragraph({
      children: [new TextRun({ text: `${r.encabezado.especialidad1} · ${r.encabezado.especialidad2}`, size: 18, color: "555555" })],
    })
  );
  ...
  const nombre = (r.paciente.nombre || "paciente").replace(/[^\w\s-]/g, "").trim().replace(/\s+/g, "_");
  saveAs(blob, `Reporte_US_${nombre}_${r.fechaReporte.replace(/\//g, "-")}.docx`);
}
