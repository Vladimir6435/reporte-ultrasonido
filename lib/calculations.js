...
/**
 * Edad en años cumplidos a una fecha de referencia (fecha del ultrasonido).
 * @param {string} dobISO  Fecha de nacimiento (YYYY-MM-DD)
 * @param {string} refISO  Fecha de referencia (YYYY-MM-DD)
 */
...
export const CRL_ESTANDAR_LABEL = {
  intergrowth: "INTERGROWTH-21st",
  robinson: "Robinson-Fleming",
};

/**
 * Edad gestacional (días) a partir de la longitud cefalocaudal (LCC/CRL).
 * - INTERGROWTH-21st (Papageorghiou 2014): se invierte la ecuación oficial
 *     CRL(mm) = −50.6562 + 0.815118·EG + 0.00535302·EG²   (EG en días)
 * - Robinson & Fleming 1975: EG(días) = 8.052·√(CRL) + 23.73
 * @param {number} crlMm  longitud cefalocaudal en milímetros
 * @param {string} estandar  "intergrowth" | "robinson"
 */
export function crlToGADays(crlMm, estandar = "intergrowth") {
  const crl = Number(crlMm);
  if (!crl || crl <= 0) return null;
  if (estandar === "robinson") {
    return Math.round(8.052 * Math.sqrt(crl) + 23.73);
  }
...
/**
 * Edad gestacional (días) a partir del diámetro biparietal.
 * Hadlock 1984: EG(semanas) = 9.54 + 1.482·DBP + 0.1676·DBP²  (DBP en cm)
 * La datación por biometría en 2.º trimestre es prácticamente
 * independiente de la población, por lo que se usa este estándar universal.
 * @param {number} bpdMm  diámetro biparietal en milímetros
 */
export function bpdToGADays(bpdMm) {
  const cm = Number(bpdMm) / 10;
  if (!cm || cm <= 0) return null;
  const weeks = 9.54 + 1.482 * cm + 0.1676 * cm * cm;
...
export function computeEG(egCalc, refISO) {
  ...
  if (egCalc.metodo === "fur") {
    ...
    detalle = { confiable: egCalc.furConfiable || "confiable" };
    ...
    detalle = { fecha: egCalc.usFecha, gaScanDays: base };
    ...
    fpp = addDays(ref, TERM_DAYS - totalDays);
  } else if (egCalc.metodo === "bpd") {
    const bp = parseDate(egCalc.bpdFecha);
    if (!bp) return { ok: false, error: "Ingrese la fecha de la medición del DBP." };
    const gaScan = bpdToGADays(egCalc.bpd);
    if (gaScan == null) return { ok: false, error: "Ingrese un diámetro biparietal válido (mm)." };
    totalDays = gaScan + diffDays(ref, bp);
    fpp = addDays(ref, TERM_DAYS - totalDays);
    detalle = { bpd: egCalc.bpd, fecha: egCalc.bpdFecha, gaScanDays: gaScan };
  } else if (egCalc.metodo === "crl") {
    const cf = parseDate(egCalc.crlFecha);
    if (!cf) return { ok: false, error: "Ingrese la fecha de la medición de LCC." };
    const gaScan = crlToGADays(egCalc.crl, egCalc.crlEstandar);
    if (gaScan == null) return { ok: false, error: "Ingrese una longitud cefalocaudal válida (mm)." };
    totalDays = gaScan + diffDays(ref, cf);
    fpp = addDays(ref, TERM_DAYS - totalDays);
    detalle = { crl: egCalc.crl, fecha: egCalc.crlFecha, gaScanDays: gaScan, estandar: egCalc.crlEstandar };
  } else {
    return null;
  }
