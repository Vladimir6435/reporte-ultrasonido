...
function antecedentesMedicos(h_) {
  const items = [];
  if (h_.htaCronica === "si") items.push("Hipertensión arterial crónica");
  if (h_.diabetes && h_.diabetes !== "no") {
    const map = { tipo1: "Diabetes tipo 1", tipo2: "Diabetes tipo 2", gestacional: "Diabetes gestacional" };
    items.push(map[h_.diabetes]);
  }
  if (h_.colageno === "si") items.push("Enfermedad del colágeno" + (h_.colagenoCual ? ` (${h_.colagenoCual})` : ""));
  if (h_.medicamentos === "si") {
    ...
    items.push("Medicamentos: " + (meds.length ? meds.join(", ") : "sí"));
  }
...
function antecedentesObstetricos(h_) {
  ...
  if (h_.cesareas === "si")
    items.push({ label: "Cesárea previa", value: "Sí" + (h_.cesareasFechas ? ` (${h_.cesareasFechas})` : "") });
  else items.push({ label: "Cesárea previa", value: "No" });
  ...
  items.push({
    label: "Embarazo anterior",
    value: ant.length ? ant.join(", ") : "sin antecedentes de pretérmino, preeclampsia ni RCIU",
  });
  return items;
}
...
function findingsOpcion3(state, blocks) {
  const o = state.op3;
  blocks.push(h("Evaluación morfológica (22–24 semanas)"));
...
function findingsOpcion4(state, blocks) {
  ...
  // Doppler
  const dop = [
    { label: "Arterias uterinas", value: o.uterinasNm ? "No se requiere" : o.uterinas },
    { label: "Arteria umbilical", value: o.umbilicalNm ? "No se requiere" : o.umbilical },
    { label: "Arteria cerebral media", value: o.acmNm ? "No se requiere" : o.acm },
    { label: "Índice cerebro-placentario", value: o.icpNm ? "No se requiere" : o.icp },
    { label: "Ductus venoso", value: o.dvNm ? "No se requiere" : o.ductusVenoso },
...
export function buildNotaMedica(state) {
  ...
  if (r.edadGestacional) L.push(`EG: ${r.edadGestacional}${r.fpp ? ` | FPP: ${r.fpp}` : ""}`);
