...
import {
  ...
  diffDays, classifyRCIU, computeEG, CRL_ESTANDAR_LABEL,
} from "./calculations";
import {
  ANATOMIA_22_24, ANATOMIA_11_14, UBICACION_PLACENTA, LIQUIDO_AMNIOTICO, ENCABEZADO,
} from "./constants";
import { ESTANDAR_LABEL } from "./percentile";

const pesoPctTxt = (pct, estandar) =>
  pct ? `p${pct} (${ESTANDAR_LABEL[estandar] || ""})` : "";

function liquidoTxt(o) {
  const cual = labelFrom(LIQUIDO_AMNIOTICO, o.liquidoAmniotico);
  if (o.liquidoValor) {
    const t = o.liquidoTipo === "bvm" ? "bolsillo vertical máximo" : "ILA";
    return `${cual} (${t} ${o.liquidoValor} mm)`;
  }
  return cual;
}
...
// ============================================================
// Texto que explica el origen del cálculo de edad gestacional.
function fuenteEG(r) {
  const d = r.detalle || {};
  switch (r.metodo) {
    case "fur":
      return `Calculada por fecha de última regla (${d.confiable === "no_confiable" ? "no confiable" : "confiable"}).`;
    case "us":
      return `Calculada por ultrasonido previo realizado el ${formatDate(parseDate(d.fecha))}, con ${daysToWeeksString(d.gaScanDays)} en ese momento.`;
    case "fpp":
      return "Calculada a partir de la fecha probable de parto.";
    case "manual":
      return "Método manual ingresado por el médico.";
    case "bpd":
      return `Calculada por diámetro biparietal de ${d.bpd} mm (${daysToWeeksString(d.gaScanDays)} al momento de la medición, ${formatDate(parseDate(d.fecha))}).`;
    case "crl":
      return `Calculada por longitud cefalocaudal de ${d.crl} mm (${daysToWeeksString(d.gaScanDays)}, ${CRL_ESTANDAR_LABEL[d.estandar] || ""}).`;
    default:
      return `Calculada por ${r.metodoLabel}.`;
  }
}
...
function findingsOpcion1(state, blocks) {
  ...
  if (o.vesiculaVitelina) items.push({ label: "Vesícula vitelina", value: o.vesiculaVitelina });
  const luteoMap = { ovario_izq: "Presente en ovario izquierdo", ovario_der: "Presente en ovario derecho", ausente: "Ausente" };
  items.push({ label: "Cuerpo lúteo", value: luteoMap[o.cuerpoLuteo] || "" });
...
function findingsOpcion3(state, blocks) {
  ...
  if (o.fcf) blocks.push(kv([{ label: "Frecuencia cardíaca fetal", value: `${o.fcf} lpm` }]));
  ...
  if (o.incluirBiometria) {
    const b = [
      ...
      { label: "Percentil de peso", value: pesoPctTxt(o.pesoPercentil, state.percentilEstandar) },
    ];
  ...
  blocks.push(kv([
    ...
    { label: "Líquido amniótico", value: liquidoTxt(o) },
  ].filter(Boolean)));
...
function findingsOpcion4(state, blocks) {
  ...
  blocks.push(kv([
    { label: "Frecuencia cardíaca fetal", value: o.fcf ? `${o.fcf} lpm` : "" },
    { label: "DBP", value: o.dbp ? `${o.dbp} mm` : "" },
    { label: "CC", value: o.cc ? `${o.cc} mm` : "" },
    { label: "CA", value: o.ca ? `${o.ca} mm` : "" },
    { label: "LF", value: o.lf ? `${o.lf} mm` : "" },
  ...
  if (o.placentaAnormal) {
    ...
    blocks.push(alert("Placenta anormal", txt));
  }
...
// ============================================================
export function getMedico(state) {
  const lista = state.medicos || [];
  return lista.find((m) => m.id === state.medicoSelId) || lista[0] || { nombre: "", especialidades: "" };
}
...
export function buildNotaMedica(state) {
  ...
  L.push(`\n${r.encabezado.medico}${r.encabezado.especialidades ? ` — ${r.encabezado.especialidades}` : ""}`);
  return L.join("\n");
}
